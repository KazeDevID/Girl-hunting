...
Imported['LimitedOneItem'] = 1.00;

/*:
 * @plugindesc ver1.00/1つしか所持できないアイテムを設定できるようにします。
 * @author Yana
 * 
 * @param LimitedKey
...
(function () {
	...
	var limitedKey = RegExp(parameters['LimitedKey'] || '<限定品>');
	...
	var ungettableMessage = String(parameters['UngettableMessage'] || '「\\I[_icon]_name」は\\c[4]これ以上持てない！');
	...
	Game_Party.prototype.maxItems = function (item) {
		if (DataManager.isLimitedOne(item)) {
			return 1
		}
	...
	Game_Party.prototype.gainItem = function (item, amount, includeEquip) {
		...
		if (item !== null) { //check of item = null
			if ((amount+n) >= item.maxItems) {
				let temp = item.maxItems-n;
